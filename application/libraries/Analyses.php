...
class Analyses
{
        ...
        public function check_amount($data){
         ...
         public function getPaymentSlip($strJson, $DTI, $top_bottom=NULL, $type, $name_input=NULL)
        {
         ###################### ANALYTICS VARIABLES ##################
         $input= str_replace("\\","",$name_input);
         $input = json_decode(preg_replace(["/\"?(\w+)\"?=>/", "/[\r\n]|\s{2,}/"], ['"$1":', ''], $input), true);
       //  $input= json_decode($name_input, true);
        $bank_name=' ';
	    $opening_balance=' ';
        $closing_balance=' ';
        $lender_name=' ';
        ...
        $name_status=false;
        $msg='';
        $o_input=json_encode($input['bvn']);

        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, 'https://mobile.creditclan.com/webapi/v1/get_bvn_data/');
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS, $o_input);
  ...
  if(!empty($top_bottom)){
           ...
           $json = $json['top_bottom_data'];
            ...
            foreach($result_top as $tb){ $tb_n=0; $tb_a++;
                if(strpos(strtolower($tb['str']), 'bank') !== false) {
                    $extract_bank=explode(' ', $tb['str']);
                ...
                if(strpos(strtolower($tb['str']), ':') !== false){
                    $tbx=explode(': ', $tb['str']);
                    if(count($tbx)>1){
                        $tb_name=$tbx[1];
                    ...
                    $tbx2=explode(' ', $tb['str']);
                        $t_date= $this->test_date($tb['str']);
                        $t_am= $this->check_amount($tb['str']);
                    if($tbx2 > 1 && $tbx2 < 4 && $t_date == false && $t_am==false){
                        if (!preg_match($pattern_air, $tb['str'])) {
                            $lender_name=$tb;
                        }
                    }
                }
               
                if(strpos(strtolower($tb['str']), 'account number') !== false || strpos(strtolower($tb['str']), 'account no') !== false) {
                    $tbn=explode(' ', $tb['str']);
                ...
                if(strpos(strtolower($tb['str']), 'opening balance') !== false || strpos(strtolower($tb['str']), 'opening bal') !== false) {
                    $opening_balance=$result_top[$tb_a];
                }
    
                if(strpos(strtolower($tb['str']), 'closing balance') !== false || strpos(strtolower($tb['str']), 'closing bal') !== false) {
                    $closing_balance=$result_top[$tb_a];
                }
            }

            foreach($result_top as $tb){
            $arr1=explode(' ', $input);
            $arr2=explode(' ', $tb['str']);
            $aint=array_intersect($arr2, $arr1);
            $name_score=(count($aint) * 100 / count($arr2));
            ...
            if($name_score > 39 && $name_score < 60){
                $msg='Partially Found';
                $name_status=true;
            break;
            }
        ...
        $msg='Name can not be determined';
    }
            $date_array=[];  
            
           // $total_credit=array_sum(array_column($result, 'credit'));
...
//	print_r($total_monthly_withdraw);    
    ...
    foreach($lodgement as $loo){
        ...
        $a_name=array('status'=>$name_status, 'score'=>$name_score, 'message'=>$msg);
...
//        print_r($highlight); exit;
 ...
 if(empty($credit_month)){
        ...
        public function get_sweeper($date, $amount)
        {
            ...
            foreach ($this->out as $masterkey => $mastervalue) {
             foreach ($mastervalue as $keys) {
                ...
                if($keys['transaction_type'] == 'debit' && $mydate > $curdate){
                $a+= $keys['amount'];
                if($a>=$amount){
                    $we=$keys['date'];
                    $is_break=true;
    ...
    /*
    $dti=33.3; $type=1; $top_bottom=null;
    $homepage = file_get_contents("chk.json");
    //print_r($homepage); exit;
    $testObject = new Ana();
    
    $result=$testObject->getPaymentSlip($homepage, $dti, $top_bottom, $type);
