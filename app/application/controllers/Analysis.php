...
class Analysis extends REST_Controller {
    ...
    public function index_post()
    {
                ...
                if(!empty($ext)){
                    ...
                    if($j && !empty($j)){
                        ...
                        if($analysis_result && !empty($analysis_result)){
                            $analysis_result=json_encode(unserialize(str_replace(array('NAN;','INF;'),'0;',serialize($analysis_result))));
                            $url='http://35.192.206.227/app/bank_analysis/analysis_pdf.php?transaction_id='.$transaction;
                 $this->db->query("update extraction_transactions set analytics_pdf='".$url."' where transaction_id='".$transaction."'");
        ...
        if($trans_type == 2){
                ...
                $ext=$this->db->query("select regular_extraction_data as e from extraction_transactions where transaction_id='".$transaction."'")->row();
                ...
                if(!empty($ext)){
                    ...
                    $analysis_result=$this->analyse($j);
                    if($j){
                        $j=json_encode(unserialize(str_replace(array('NAN;','INF;'),'0;',serialize($j))));
                        $analysis_result=$this->analyse($j);
                     if($analysis_result && !empty($analysis_result)){
                        $json=json_encode(unserialize(str_replace(array('NAN;','INF;'),'0;',serialize($analysis_result))));
                        $url='http://35.192.206.227/app/bank_analysis/analysis_pdf.php?transaction_id='.$transaction;
                        $this->db->query("update extraction_transactions set analytics_pdf='".$url."' where transaction_id='".$transaction."'");
             ...
             if($trans_type == 3){
                $ext=$this->db->query("select transaction_data as e from extraction_transactions where transaction_id='".$transaction."'")->row();
                if(!empty($ext)){
                    $ext=trim($ext->e);
                    $ext=json_decode($ext);
                    $dti=33.33;
                   
                    $j=$this->analyses->getPaymentSlip($ext, $dti);
                   
                    if($j && !empty($j)){
                        $analysis_result=json_encode(unserialize(str_replace(array('NAN;','INF;'),'0;',serialize($j))));
                        $url='http://35.192.206.227/app/bank_analysis/analysis_pdf.php?transaction_id='.$transaction;
                 $this->db->query("update extraction_transactions set analytics_pdf='".$url."' where transaction_id='".$transaction."'");
                  $this->db->query("update extraction_transactions set analytics_data='".$analysis_result."' where transaction_id='".$transaction."'");
                        $this->set_response([
    ...
    public function analyse($data){
        $dti=33.33;
        $j=$this->analyses->getPaymentSlip($data, $dti);
       /* $mpdf = new \Mpdf\Mpdf();
