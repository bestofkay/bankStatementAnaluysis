...
class Analyses
{
        ...
        public function check_amount($data){
         ...
         public function getPaymentSlip($strJson, $DTI, $top_bottom=NULL)
        {
         ###################### ANALYTICS VARIABLES ##################
        $bank_name=' ';
        $opening_balance=' ';
        ...
        $account_number=' ';
         //printf("Now: %s", Carbon::now()); exit;
            ...
            $dummy_data="with a worst-case scenario amount of sample data, to get an accurate sense of its performance in the real world.";
        ############################ GENERATING ARRAY BY MONTH GROUPING #############
            ...
            $balance=floatval($element['balance']);
                    ...
                    $result_month = date("F", $time);
                    $date_result = date("j-F-Y", $time);
            ...
            $e_date=$date_array[$count_date-1];
           //
                    ...
                    if($keys['transaction_type'] == 'credit' && $keys['amount'] > 0){
                        ...
                        if($salary){
                           ...
                           $datediffs = $salary_now - $your_dates;
                            ...
                            $state=27;
                        }elseif(($is_loan || $lender_state) && ($state != 1 && $state != 4 && $state != 5 && $state != 6)){{
                            $statess='Loan';
                            $state=30;
        ...
        $total_monthly_deposit[]=array('month'=>$masterkey, 'amount'=>$monthly_deposit, 'count'=>$count_deposit, 'first_day'=> $eachday[0], 'percent_change'=>$cre_percent_change, 'last_day'=>$eachday[count($eachday)-1]);
    ...
    //echo $each_day[0].'<br>';
        ...
        $total_monthly_withdraw[]=array('month'=>$masterkey, 'amount'=>$monthly_withdraw, 'count'=>$count_withdraw, 'first_day'=> $eachday[0], 'percent_change'=>$deb_percent_change, 'last_day'=>$eachday[count($eachday)-1]);
    ...
    foreach($suspected_salary_month as $h){
        $result_sus[$h['month']][]=$h;
    }
    foreach($suspected_salary_month as $h){
        $result_susw[$h['week']][]=$h;
    }
    ...
    ##################### GET POS/ATM ##############
        ...
        {
            $pattern = '/loan|credit|obligor|lender|repayment|repymt|rpymt|rpyt|rpym|Loan|LOAN|loans|Loans|LOANS|CREDIT|OBLIGOR|LENDER|REPAYMENT|REPYMT|RPYMT|RPYT|RPYM|MFB|mfb|Principal|PRINCIPAL|CLEARING|CONTRIBUTION|Contribution|contribution|COOPERATIVE|coop|COOP|Coop|cooperative|interest|INTEREST|Interest|settlement|Settlement|SETTLEMENT|advance|ADVANCE|adv|Adv|ADV|Advance|Reimburse|REIMBURSE|Reimburse/';
            if (preg_match($pattern, $msg)) {
                return true;
            } else {
                return false;
    ...
    ############### FUNCTION TO GET LENDERS #####################
        public function get_lender($lenders, $search)
        {
            foreach ($lenders as $lend) {
                $sub=substr($lend, 0, 4);
                if (strpos(strtolower($search), strtolower($sub)) !== false) {
                    return $lend;
                    break;
                }else{
                    return false;
